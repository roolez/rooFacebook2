...
s3eResult rooFacebook2Init_platform()
{
    ...
    g_rooFacebook_getAccessToken = env->GetMethodID(cls, "rooFacebook_getAccessToken", "()Ljava/lang/String;");
    if (!g_rooFacebook_getAccessToken)
        goto fail;

    g_rooFacebook_getAccessExpires = env->GetMethodID(cls, "rooFacebook_getAccessExpires", "()J");
    if (!g_rooFacebook_getAccessExpires)
        goto fail;

    g_rooFacebook_setAccessToken = env->GetMethodID(cls, "rooFacebook_setAccessToken", "(Ljava/lang/String;)V");
    if (!g_rooFacebook_setAccessToken)
        goto fail;

    g_rooFacebook_setAccessExpires = env->GetMethodID(cls, "rooFacebook_setAccessExpires", "(J)V");
    if (!g_rooFacebook_setAccessExpires)
        goto fail;

    g_rooFacebook_extendAccessToken = env->GetMethodID(cls, "rooFacebook_extendAccessToken", "()V");
...
int64 rooFacebook_getAccessExpires_platform(rooFacebook_Session * facebook)
{
    JNIEnv* env = s3eEdkJNIGetEnv();
    return (int64)env->CallLongMethod(g_Obj, g_rooFacebook_getAccessExpires, (jobject)facebook);
}
...
void rooFacebook_setAccessExpires_platform(rooFacebook_Session * facebook, int64 access_expires)
{
    JNIEnv* env = s3eEdkJNIGetEnv();
